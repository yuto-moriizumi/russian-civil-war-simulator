---
name: spawn
description: Create a new git worktree and branch for parallel development. Use this when asked to spawn a worktree, create a parallel branch, or start a new task in isolation.
allowed-tools: shell
---
...
2. **Create branch name**: Convert the task name to a valid git branch name (lowercase, replace spaces with hyphens, remove special characters). Use format: `task/<branch-name>` or just `$ARGUMENTS` if it already looks like a branch name.

3. **Determine worktree path**: Create the worktree inside the current working directory at `.worktrees/<branch-name>`. Create the `.worktrees` directory if it doesn't exist.
...
7. **Continue working in the new worktree**: From this point forward, use the new worktree path as the working directory for all subsequent commands and file operations.

8. **Start the task**: Begin working on the task described in $ARGUMENTS immediately.

## Important notes:
- Continue using the current session — no need to launch a new instance
