/**
 * Adjacency Building Script
 * 
 * 結合されたGeoJSONから隣接関係を抽出する
 * 
 * Usage: npx tsx scripts/build-adjacency.ts
 */
...
async function main() {
  console.log('=== Adjacency Building Script ===\n');
  
  const config = loadConfig(__dirname);
  const tempDir = path.join(__dirname, 'temp');
  
  // Step 1: 各国のGeoJSONを結合（隣接関係の検出に必要）
  console.log('Step 1: Loading GeoJSON files...');
  ...
  // Step 4: 異なる国間の隣接関係を補完
  console.log('Step 4: Detecting cross-border adjacency...');
  ...
  // Step 5: 同じ国内の隣接関係を補完
  console.log('Step 5: Detecting same-country adjacency (missed by arc-sharing)...');
  ...
  // Step 6: 孤立したリージョンの隣接関係を検出
  console.log('Step 6: Detecting isolated region adjacency...');
  ...
  // Step 7: カスタム隣接関係を適用
  console.log('Step 7: Applying custom adjacency...');
  ...
  // Step 8: 隣接関係を出力
  console.log('Step 8: Saving output file...');
  
  const adjacencyOutputPath = path.resolve(__dirname, config.output.adjacency);
  
  // 出力ディレクトリを確認
  const outputDir = path.dirname(adjacencyOutputPath);
