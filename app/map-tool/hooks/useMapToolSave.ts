...
export function useMapToolSave({
  ...
  const handleSave = useCallback(async () => {
    setIsSaving(true);
    try {
      const [ownershipRes, coreRes, unitRes, valuesRes] = await Promise.all([
        ...
        fetch('/api/map-tool/save-region-values', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ regionValues }),
        }),
      ]);
      const [ownershipResult, coreResult, unitResult, valuesResult] = await Promise.all([
        ownershipRes.json(), coreRes.json(), unitRes.json(), valuesRes.json(),
      ]);

      if (ownershipResult.success && coreResult.success && unitResult.success && valuesResult.success) {
        alert(`Successfully saved!\nOwnership: ${ownershipResult.message}\nCore Regions: ${coreResult.message}\nUnit Placement: ${unitResult.message}\nRegion Values: ${valuesResult.message}`);
        onSaveSuccess({ ownership, coreRegions, unitPlacement, armyGroupDefs, regionValues });
      } else {
        const errors: string[] = [];
        ...
        if (!valuesResult.success) errors.push(`Region Values: ${valuesResult.message}`);
        alert(`Save failed:\n${errors.join('\n')}`);
      }
    } catch (error) {
  ...
  }, [ownership, coreRegions, unitPlacement, armyGroupDefs, regionValues, onSaveSuccess]);
